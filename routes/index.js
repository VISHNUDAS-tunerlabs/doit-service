...
const pagination = require(path.join(
  PROJECT_ROOT_DIRECTORY,
  'generics',
  'middleware',
  'pagination',
...
const fs = require('fs');
const inputValidator = require(path.join(
  PROJECT_ROOT_DIRECTORY,
  'generics',
  'middleware',
  'validator',
...
module.exports = function (app) {
  const applicationBaseUrl = process.env.APPLICATION_BASE_URL || '/doit/';
    ...
    if (!req.params.version) {
      ...
      try {
        ...
        if (req.params.file) {
          ...
          result = await controllers[req.params.version][req.params.controller][req.params.method](
            req,
          );
        }
