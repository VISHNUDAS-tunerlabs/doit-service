  ...
  get(url, options, path) {
    ...
    if (options.queryParameters) {
      ...
      if (countOfQueryParameters > 0) {
        ...
        for (
          let pointerToQueryParamtersKeyArray = 0;
          pointerToQueryParamtersKeyArray < countOfQueryParameters;
          pointerToQueryParamtersKeyArray++
        ) {
    ...
    return new Promise(function (resolve, reject) {
      try {
        if (options.form) {
          ...
          else options.headers['content-length'] = Buffer.byteLength(data);
        }
      } catch (error) {
